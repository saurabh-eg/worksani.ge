...
import { Send, Star, MessageSquare, CheckCircle, Loader2, UserCircle } from 'lucide-react';
...
const ProjectBidsSection = ({ project, user, users, onBidSubmit, onAcceptBid }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8">
      ...
      {user.role === 'supplier' ? (
        // Supplier: Bids Placed section
        project.bids && project.bids.filter(b => b.supplierId === user.id).length > 0 ? (
          <div>
            <h2 className="text-2xl font-semibold mb-4 text-purple-700 dark:text-purple-400">
              {(bidsT.bidsPlacedTitle || 'Bids Placed ({count})').replace('{count}', project.bids.filter(b => b.supplierId === user.id).length)}
            </h2>
            <div className="space-y-4">
              {project.bids
                .filter(b => b.supplierId === user.id)
                      ...
                      <CardFooter className="bg-gray-50 dark:bg-slate-700/50 p-3">
                        <div className="flex justify-end space-x-3 w-full">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate('/messages', { state: { prefillChatWith: project.customerId, prefillSubject: `${messagesPageT.projectSubjectPrefix || 'Regarding Project:'} ${project.title}` } })}
                            className="dark:text-purple-300 dark:border-purple-500 dark:hover:bg-purple-700/30"
                          >
          ...
          <div className="mt-8 p-4 bg-purple-100 dark:bg-purple-900/30 rounded-lg text-center text-purple-700 dark:text-purple-300">
            {bidsT.noBidsPlacedMessage || "You haven't placed any bids on this project yet."}
          </div>
        )
      ) : (
        // Customer: Bids Received section
        project.bids && project.bids.length > 0 && (
          <div>
            <h2 className="text-2xl font-semibold mb-4 text-purple-700 dark:text-purple-400">
              {(bidsT.bidsReceivedTitle || 'Bids Received ({count})').replace('{count}', project.bids.length)}
            </h2>
            <div className="space-y-4">
              {project.bids
                        ...
                        <CardFooter className="bg-gray-50 dark:bg-slate-700/50 p-3">
                          <div className="flex justify-end space-x-3 w-full">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => navigate('/messages', { state: { prefillChatWith: bid.supplierId, prefillSubject: `${messagesPageT.projectSubjectPrefix || 'Regarding Project:'} ${project.title}` } })}
                              className="dark:text-purple-300 dark:border-purple-500 dark:hover:bg-purple-700/30"
                            >
                              <MessageSquare size={16} className="mr-2" /> {bidsT.chatButton || 'Chat'}
                            </Button>
