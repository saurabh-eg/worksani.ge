...
const MessagesPage = () => {
  ...
  }, [user, getChatPartners, currentMessages]); 
  ...
  useEffect(() => {
    if (selectedChatPartner && user) {
      const chatMessages = getMessagesForChat(user.id, selectedChatPartner.id);
      setCurrentMessages(chatMessages);
      markMessagesAsRead(selectedChatPartner.id);
    } else {
      setCurrentMessages([]);
    }
  }, [selectedChatPartner, user, getMessagesForChat, markMessagesAsRead]);
  ...
  const handleSendMessage = (e) => {
    ...
    setNewMessage('');
    const updatedMessages = getMessagesForChat(user.id, selectedChatPartner.id); // Re-fetch immediately for optimistic update
            ...
            {selectedChatPartner ? (
              <>
                ...
                <div className="flex-grow p-4 space-y-4 overflow-y-auto bg-gradient-to-br from-purple-50 to-green-50">
                  {currentMessages.map(msg => (
                    <div key={msg.id} className={`flex ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl shadow ${msg.senderId === user.id ? 'bg-purple-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-purple-100'}`}>
                        <p className="text-sm">{msg.text}</p>
                        <p className={`text-xs mt-1 ${msg.senderId === user.id ? 'text-purple-200 text-right' : 'text-gray-400 text-left'}`}>
                ...
                <form onSubmit={handleSendMessage} className="p-4 border-t border-purple-200 bg-white flex items-center space-x-3">
                  ...
                  />
                  <Button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white rounded-full p-3">
                    <Send size={20} />
                  </Button>
                </form>
              </>
